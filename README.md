# 压测工具如何选择? ab、locust、Jmeter、go实现压测的工具【单台机器100w连接压测实战】

本文介绍压测是什么，解释压测的专属名词，教大家如何计算压测指标，介绍市面上的常见压测工具(ab、locust、Jmeter、go实现压测工具、云压测)，如何选择一款适合自己的压测工具，本文还有两个压测实战
...
- [4、go-stress-testing go语言实现的压测工具](#4go-stress-testing-go语言实现的压测工具)
    ...
    - [4.2 用法](#42-用法)
    - [4.3 实现](#43-实现)
    - [4.4 go-stress-testing、ab对Golang web 压测](#44-go-stress-testing、ab对Golang-web-压测)
- [5、压测工具的比较](#5压测工具的比较)
    - [5.1 比较](#51-比较)
...
![go-stress-testing01](![//img.mukewang.com/5d64a48e0001bb8421170573.png)
...
之前写了一篇文章，[基于websocket单台机器支持百万连接分布式聊天(IM)系统](https://github.com/link1st/gowebsocket)，单台机器支持100W连接，这里对这个项目进行压测
...
被压测服务器需要保持100W长连接，客户和服务器端是通过socket通讯的，每个连接需要建立一个socket，程序需要保持100W长链接就需要单个程序能打开100W个文件句柄
...
1024一下是系统保留端口，所以能使用的1024到65535

如果需要100W长连接 100W / (65535-1024) ≈ 15.5，所以这里需要16台服务器
...
- 2. 查看gowebsocket服务器的内网端口

- 3. 登录上16台压测服务器，这里我提前把需要优化的系统做成了镜像，申请机器的时候就可以直接使用这个镜像
...
建立连接以后，`-n 1`发送一个**ping**的消息给服务器，收到结果以后保持连接不中断

- 5. 通过 gowebsocket服务器 的http接口，实时查询连接数和项目启动的协程数
...
压测服务16台 压测完成的统计数据 62500 是每个客户端连接的数量 `62500*16=100W`
...
如果需要优化内存可以参考**@Roy11568780**给的解决方案
...
单台机器支持100W连接是可以的，但是实际业务比较复杂，还是需要持续优化~
